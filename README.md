...
- [ ] Add a landscape mode and do matrix math to animate the board
...
3. Copy the `pico-sdk/external/pico_sdk_import.cmake` file from the pico-sdk directory into the root of project folder
4. Add `PICO_SDK_PATH=path/to/sdk` to your environment variables
5. Make sure to include the following at the top of your root `CMakeLists.txt`
```cmake
...
The FFT is set to run for 8 LED strips and an FFT size of 128. You can configure the FFT settings for your implementation in [fft_tools.h](/src/utils/include/utils/fft_tools.h) and the LED settings in [ws2812_config.h](/src/utils/include/utils/ws2812_config.h)
